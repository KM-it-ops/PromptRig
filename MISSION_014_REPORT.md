...
**Status:** OAR-008 Accepted 2026-08-14.  
...
6. This report, OAR-008, maturity map updates (Requirements compiler stays `PARTIAL`)
...
| Governance | OAR-008 Accepted 2026-08-14, maturity map, certification README |
...
| `tests/compiler/test_model_suggest.py` | Deterministic proposal; no input mutation; no provider/HTTP imports; MAS-GATE-0001/0002 including nested records |
| `tests/compiler/test_model_suggest_closed_loop.py` | Default off; IR ids unchanged with sidecar; self-accept INVALID_OUTPUT; JSON flag IR-identical to options |
...
**Verification command:** `uv run python -m pytest tests/compiler tests/evaluation -q`
...
Live execution, API keys on the certified path, freeform NLP, live model calls, Simple Mode UI semantics, benchmark results, CERTIFIED requirements compiler, and production hosted surfaces remain unauthorized.

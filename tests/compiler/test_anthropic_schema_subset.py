...
def test_documented_but_unenforced_keywords_are_not_flagged_as_violations():
    """minLength/pattern/minimum/etc. are confirmed accepted-but-silently-
    unenforced by Anthropic's constrained decoding under strict mode, not
    rejected -- so this checker must not treat their presence as a
    violation (see module docstring)."""
    schema = {
        "type": "object",
        "additionalProperties": False,

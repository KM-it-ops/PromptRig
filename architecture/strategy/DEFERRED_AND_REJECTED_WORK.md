...
- MISSION-017 added file/api envelope producers for canonical assembly (`produce_requirements`, `compile_requirements_input`); MISSION-018 added simple/developer envelope producers; MISSION-019 added structured prs envelope producer (PRS language/grammar still DEFERRED per `PRS_DISPOSITION.md`); MISSION-020 added authoring-prose constrained `plain_language_v0` lowerer (OAR-014 Accepted 2026-08-22); OQ-008-001 through OQ-008-010 are owner-resolved in `OPEN_QUESTIONS.md` (policy only; engine still fail-closed until a new campaign implements them); remaining unauthorized: OQ implementation, no full 008 compiler; no M3; freeform NLP.

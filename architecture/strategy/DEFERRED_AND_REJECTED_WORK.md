...
- MISSION-017 added file/api envelope producers for canonical assembly (`produce_requirements`, `compile_requirements_input`); MISSION-018 added simple/developer envelope producers; MISSION-019 added structured prs envelope producer (PRS language/grammar still DEFERRED per `PRS_DISPOSITION.md`); OQ-008-001 through OQ-008-010 are owner-resolved in `OPEN_QUESTIONS.md` (policy only; engine still fail-closed until a new campaign implements them); remaining unauthorized: authoring-prose producers; no full 008 compiler; no M3.

...
3. **M2 (implemented, awaiting OAR-008 — MISSION-014):** Optional headless `fake-suggester-v0` suggestion sidecar (see `architecture/mission-014-certification/FAKE_SUGGESTER.md`); proposals remain `acceptance_state=proposed` and are never mapped to IR; cannot bypass deterministic validation, authority/defaults, or evidence rules; not a live provider. OAR-008 is Ready for owner acceptance — not Accepted.

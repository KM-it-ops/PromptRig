...
3. **M2 (authorized, in progress — MISSION-014):** Optional headless `fake-suggester-v0` suggestion sidecar (see `architecture/mission-014-certification/FAKE_SUGGESTER.md`); proposals remain `acceptance_state=proposed` and are never mapped to IR; cannot bypass deterministic validation, authority/defaults, or evidence rules; not a live provider. M2 is not done until Task 6 certification.
